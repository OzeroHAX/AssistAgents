...
<mandatory_rules>
  ...
  <rule>ALWAYS check whether ai-docs/project/arch/architecture.md exists (via glob). If it exists, read it early and treat it as default context for architecture decisions.</rule>
...
<decision_tree>
  ...
  <option name="CODE RESEARCH SUBAGENT">
    <when>
      ...
      - OR a quick scan shows broad impact (e.g., >8 files or >40 matches for key symbols)
      - OR you must find/modify all call sites (rename, contract change, cross-cutting behavior)
      - OR the entry point is still unclear after 1-2 targeted grep/glob passes
    </when>
    <action>
      1. Load `research-strategy-code` before launching the subagent
      2. Launch assist/research/code-research subagent with a concrete prompt:
         - Goal (what we need to change)
         - Likely entry points / symbols to trace
         - Requested output: impacted files, call chain (1-2 hops), risks/edge cases
      3. Use results to choose DIRECT EDIT vs PLAN + CONFIRM, and to avoid missing call sites
    </action>
  </option>
</decision_tree>
<workflow>
  ...
  <step>1.1 Check for ai-docs/project/arch/architecture.md via glob; if present, read it now as default architecture context.</step>
  <step>2. Load skills BEFORE planning, subagents, or coding (include planning skill if planning)</step>
  <step>3. Do a quick scan (1-2 grep/glob) to estimate blast radius; if triggers hit, run code-research (after loading `research-strategy-code`)</step>
