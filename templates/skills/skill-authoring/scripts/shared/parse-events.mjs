...
function tryReadUsageCandidate(value) {
  ...
  const total = numberOrNull(value.totalTokens ?? value.total_tokens ?? nestedTokens?.total ?? nestedTokens?.total_tokens);
...
function looksLikeToolCall(record, pathLabel) {
  const typeHints = [
...
function extractSkillToolMatches(record, knownSkillNames) {
  ...
  const candidates = [
    record.state?.metadata?.name,
    record.metadata?.name,
    record.state?.input?.name,
    record.input?.name,
  ].filter((value) => typeof value === 'string' && value.trim().length > 0);

  const outputText = typeof record.state?.output === 'string'
    ? record.state.output
    : (typeof record.output === 'string' ? record.output : '');
  const outputSkillNames = Array.from(outputText.matchAll(/<skill_content name="([^"]+)"/g))
    .map((match) => match[1])
    .filter((value) => typeof value === 'string' && value.trim().length > 0);
...
function collectAssistantText(value, assistantContext, collected) {
  ...
  if (lowerString(value.type) === 'text' && typeof value.text === 'string') {
...
function extractSessionIds(value, collected) {
  if (Array.isArray(value)) {
...
function walkEvent(value, pathStack, knownSkillNames, toolCalls, skillCalls, usageCandidates) {
  if (Array.isArray(value)) {
...
export function parseEventText(rawContent, knownSkillNames) {
