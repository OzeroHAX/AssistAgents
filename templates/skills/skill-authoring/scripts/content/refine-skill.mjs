...
export function buildRefinerPrompt(skillText, diagnosis) {
  return [
    ...
    '- shorten and deduplicate wording wherever possible without losing trigger precision or execution clarity',
    '- aim for the smallest valid behavioral contract, not the most exhaustive prose',
    '- if two nearby bullets say nearly the same thing, merge them into one stronger sentence',
    '- avoid restating the same idea across purpose, when_to_use, output_requirements, and validation unless the section adds distinct signal',
